...
describe('BudgetDetailsPanel Component (Appreciate)', () => {
  ...
  it('renders correctly and switches between Amortization Schedule and Comparative Analysis tabs', async () => {
    ...
    vi.mocked(store.getInstrumentName).mockImplementation((id: string) => {
      if (id === 'inst-1') return 'Pension Plan';
      if (id === 'totals') return 'All Instruments';
      return id;
    });
    ...
    vi.mocked(store.getInstrumentComparativeAnalysis).mockReturnValue({
      'Final Balance': { b1: '$300,000' },
      'Interest Growth': { b1: '$180,000' },
    });
    ...
    expect(wrapper.text()).toContain('$600.00/mo');
    expect(wrapper.text()).toContain('Planning Phase');
    expect(wrapper.text()).toContain('Career Accumulation');

    // Instrument dropdown exists
    const menu = wrapper.findComponent(BaseMenu);
    expect(menu.exists()).toBe(true);
    expect(menu.props('text')).toBe('All Instruments');
    ...
    // Crosstab content compares budgets for the selected instrument
    expect(wrapper.text()).toContain('All Instruments - Budget Comparison');
    expect(wrapper.text()).toContain('Career Budget 1');
